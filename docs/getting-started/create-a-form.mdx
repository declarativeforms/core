...
Define a form in a YAML file, push it to a GitHub repository, and Declarative Forms turns it into a live, shareable form. API-managed JSON definitions are available for automation-driven workflows.
...
<Tabs>
  ...
  <Tab title="Private repository">
    Configure the self-hosted API with a read-only `GITHUB_TOKEN`, then register
    the source through the protected management API:

    ```bash
    curl -X POST https://forms.example.com/api/v1/forms/github \
      -H "Authorization: Bearer $FORMS_API_KEY" \
      -H "Content-Type: application/json" \
      --data '{"owner":"acme","repository":"forms","path":"contact.yaml"}'
    ```

    The response contains an `a...` form ID. Open and share
    `https://forms.example.com/<form-id>`.

    <Note>
    **Your respondents never receive the GitHub token.** Private repositories
    cannot be loaded through the public owner/repository path; they must be
    registered by an API-key-authenticated management client.
    </Note>
  </Tab>
</Tabs>
...
```text
https://forms.example.com/a12345678
```
...
<Info>
**This is the URL your respondents use — not the repository path.** The short URL works without GitHub authentication, including for a registered private source.
</Info>
...
<AccordionGroup>
  <Accordion title="Form not loading">
    ...
    - For private repos, make sure the server has a `GITHUB_TOKEN` with read
