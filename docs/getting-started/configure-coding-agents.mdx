...
- Every section has an `id` and `fields`; add `title` and `next` when the form
  needs them.
- Every field has an `id`, `type`, and `label`.
- Conditional expressions reference values with `data.<field_id>`.
- Connections use `email` or `webhook` and run only when the deployment
  allowlists the source repository.
