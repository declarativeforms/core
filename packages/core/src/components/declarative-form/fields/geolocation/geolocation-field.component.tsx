...
export function GeolocationField({
  ...
  useEffect(() => {
    if (
      state.status === "idle" &&
      isGeolocationValue(controllerField.value)
    ) {
      setState({ status: "success", value: controllerField.value });
    }
  }, [controllerField.value, state.status]);
        ...
        if (position.coords.accuracy < bestAccuracyRef.current) {
          ...
          setState((prev) => {
            if (prev.status === "loading") {
              // First position received — start settle timer
        ...
        {state.status === "error" && (
          ...
          >
            <p className="flex items-center gap-2 text-sm text-destructive">
              ...
              {t(ERROR_MESSAGE_KEYS[state.code])}
            </p>
            <button
              type="button"
              onClick={requestLocation}
