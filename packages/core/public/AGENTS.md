...
| `theme` | object | `primary`, a 3- or 6-digit hex accent color, and an HTTPS `logo` URL. |
...
All 24 types. Every field also accepts `id`, `type`, `label`, `placeholder`,
`validators`, and `visible_when`. A `text_block` adds `content`; its
`placeholder` is unused and its validators are ignored.
...
| `turnstile` | Cloudflare verification | none |
| `url` | URL input | none |
...
| `text_block` | Safe Markdown content; stores an empty string | `content` |

Use `text_block` for summaries, instructions, disclosures, or terms between
questions:
...
`content` is localized text and supports `{{data.field_id}}` templates.
Headings, paragraphs, emphasis, lists, blockquotes, inline and fenced code, and
links render as Markdown. Raw HTML and Markdown images do not render, and links
must use HTTPS or `mailto:`. The optional `label` renders above the content. Its
value defaults to and persists as `""`; `placeholder` is unused and validators
are accepted but ignored. Use a separate required choice field for explicit
acceptance.
...
Use `type: turnstile` on any step for in-place Cloudflare verification. It uses
deployment-wide `TURNSTILE_SITE_KEY`, `TURNSTILE_SECRET_KEY`, and
`VERIFICATION_SECRET`; never put these in form YAML. Add `validators: [required]`
to require completion. Optional empty fields are allowed. Success stores
`"verified"` in the field and a server-issued proof in `<field-id>_token`, which
is generated automatically. Local proofs survive later steps and resume; like
email OTP, they have no expiry and can be reused across submissions.
...
`description`, a field `label` and `placeholder`, a text block's `content`,
option labels, a rating's `min_label` and `max_label`, the completion `title`,
...
Set an accent color and decorative logo under `theme`:
...
`logo` must be an absolute HTTPS image URL. It appears on the start, question,
status, and completion screens, including embeds. It is not clickable and is
hidden if the image cannot be loaded. Use only a real URL supplied or approved
by the user; never invent a placeholder logo URL.
...
- The `_token` suffix is reserved for verification proofs. Fields with that
  suffix are required while visible. Email fields with `otp: true` and
  `turnstile` fields generate and reserve `<field-id>_token` automatically.
  Optional empty Turnstile fields hide their companion. Server verification
  binds proofs to the form and source field; email proofs also bind the address.
...
- Empty defaults: `file_upload` and `multiple_select` start as `[]`; `camera`,
  `geolocation`, and `signature` start as `null`; fields of every other type,
  including `text_block`, start as `""`.
