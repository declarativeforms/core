...
- Shared validation issues or scalar unions are contracts only when an explicit
  current domain workflow consumes them. They must not exist merely to carry a
  failure category to an HTTP route.
