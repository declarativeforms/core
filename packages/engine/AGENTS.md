...
- **Add a `default` only where a shared tail genuinely absorbs many members.**
  ...
  ten generic-shaped field types legitimately share one branch. `text_block`
  uses explicit branches where it carries content, while default-value
  generation treats it like the other scalar fields.
