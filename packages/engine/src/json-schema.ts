...
const fieldBaseProperties: Record<string, JsonSchemaNode> = {
  ...
  validators: {
    type: 'array',
    description:
      'Validation rules, run in order. The first failure is the message shown. Accepted but ignored on `text_block` fields.',
...
function fieldBranch(
  ...
  return {
    ...
    required: ['id', 'type', ...extraRequired],
    properties: {
      type: { enum: [...types] },
      ...fieldBaseProperties,
      ...extraProperties,
...
const fieldBranches: Array<JsonSchemaNode> = [
  fieldBranch(
    FIELD_TYPE_GROUPS.textBlock,
    'A content field rendered in place among the questions. It follows the standard field lifecycle and persists an empty-string value. `label` is rendered, `placeholder` is unused, and validators are accepted but ignored. Use a separate required choice field when the respondent must explicitly accept terms.',
    {
      content: ref(
        'localizedText',
        'Markdown content shown to the respondent. Supports templating. Raw HTML and Markdown images are not rendered.',
      ),
    },
    ['content'],
  ),
  fieldBranch(
    FIELD_TYPE_GROUPS.turnstile,
    'Cloudflare Turnstile verification on any step. Uses deployment-wide keys and verifies in place. Stores "verified" and a generated hidden <field-id>_token proof. Add a required validator to require completion; optional empty fields are allowed. Like email OTP, local proofs do not expire and can be reused across submissions.',
  ),
  fieldBranch(
    ...
    theme: {
      ...
      properties: {
        logo: {
          ...
          pattern: '^https://',
          description:
            'Absolute HTTPS URL for a decorative brand logo shown throughout the form journey. The image is hidden if it cannot be loaded.',
